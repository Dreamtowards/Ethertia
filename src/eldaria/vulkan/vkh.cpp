...
void vkh::DestroyInstance(VkInstance instance)
{
    if (g_EnableValidationLayer) {
...
void vkh::CreateSurface(VkInstance instance, GLFWwindow* glfwWindow, VkSurfaceKHR& out_SurfaceKHR)
{
                      ...
                      VkImageUsageFlags usage, VkMemoryPropertyFlags memProperties, VkImageTiling tiling)
{
    ...
    VK_CHECK_MSG(vkCreateImage(g_Device, &imageInfo, nullptr, &image), "failed to create image.");
...
void vkh::CreateTextureImage(BitmapImage& bitmapImage, Image& img) {
...
{
    VkCommandBuffer cmdbuf = vkh::BeginCommandBuffer_Onetime();
    ...
    vkh::EndCommandBuffer_Onetime(cmdbuf);
}


void vkh::CreateTextureImage(BitmapImage& bitmapImage, VkImage& out_image, VkDeviceMemory& out_imageMemory, VkImageView* out_imageView)
{
...
void vkh::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
    VkCommandBuffer cmdbuf = BeginCommandBuffer_Onetime();
