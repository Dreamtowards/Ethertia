...
public:
    ...
    static void RenderKey(ImVec2& p, const char* name, float keySizeX = 1.0f, float sizeAddGaps = 0)
    {
    ...
    static void ShowKeymap()
    {
        ...
        RenderKey(p, "ESC"); p.x += _keysize.x + _keygap;
        RenderKey(p, "F1");
        RenderKey(p, "F2");
        RenderKey(p, "F3");
        RenderKey(p, "F4"); p.x += 0.5f * (_keysize.x + _keygap);
        RenderKey(p, "F5");
        RenderKey(p, "F6");
        RenderKey(p, "F7");
        RenderKey(p, "F8"); p.x += 0.5f * (_keysize.x + _keygap);
        RenderKey(p, "F9");
        RenderKey(p, "F10");
        RenderKey(p, "F11");
        RenderKey(p, "F12");
...
#define STEP_ROW p.x = begin.x; p.y += _keysize.y + _keygap;
        ...
        RenderKey(p, "_\n-");
        RenderKey(p, "+\n=");
        RenderKey(p, "backspace", 2.0f, 1);

        STEP_ROW
        RenderKey(p, "tab", 1.5f);
        RenderKey(p, "Q");
        RenderKey(p, "W");
        RenderKey(p, "E");
        RenderKey(p, "R");
        ...
        RenderKey(p, ":\n;");
        RenderKey(p, "\"\n'");
        RenderKey(p, "return", 2.25f, 1);

        STEP_ROW;
        RenderKey(p, "shift", 2.0f, 1);
        RenderKey(p, "Z");
        RenderKey(p, "X");
        RenderKey(p, "C");
        RenderKey(p, "V");
