...
std::string json::jobject::dump(unsigned int indent_level) const
{
    std::string result = "";
    ...
    if (is_array()) {
        ...
        for (size_t i = 0; i < this->size(); i++)
        {
            switch(json::jtype::peek(*this->data.at(i).second.c_str())) {
                case json::jtype::jarray:
                case json::jtype::jobject:
                    result += json::jobject::parse(this->data.at(i).second).dump(indent_level + 1);
                    break;
                default:
        ...
        for (size_t i = 0; i < this->size(); i++)
        {
            ...
            switch(json::jtype::peek(*this->data.at(i).second.c_str())) {
                case json::jtype::jarray:
                case json::jtype::jobject:
                    result += std::string(json::parsing::tlws(json::jobject::parse(this->data.at(i).second).dump(indent_level + 1).c_str()));
                    break;
                default:
                    result += this->data.at(i).second;
                    break;
