...
namespace json
{
	...
	public:
		 ...
		 */
		bool operator== (const json::jobject other) const { return this->as_string() == other.as_string(); }
		...
		public:
            ...
            /*! \brief Comparison operator */
			bool operator== (const std::string other) const { return this->as_string() == other; }
			...
			operator bool() const { return this->is_bool() && this->ref() == "true"; }
		...
		public:
			...
			{
				this->sink.set(key, input.as_string());
			}
			...
			void operator=(const std::vector<json::jobject>& input)
			{
				...
				{
					objs.push_back(input[i].as_string());
				}
		 ...
		 */
		std::string dump(unsigned int indent_level = 0) const;
	};
}
