...
public:
    ...
    template<typename T>
    bool get(const string_t& key, T& out)
    {
        if (this->exists(key))
        {
            out = (*this)[key].as<T>();
    ...
    auto get_multi(key_then_default_value_t&&... keys_then_default_value) const;
    ...
    basic_value<string_t>& operator=(const std::vector<string_t>& rhs)
    {
        (*this) = json::array(rhs);
        return *this;
    }
...
template <typename string_t>
template <typename... key_then_default_value_t>
MEOJSON_INLINE auto basic_value<string_t>::get_multi(key_then_default_value_t&&... keys_then_default_value) const
{
    return get_multi(std::forward_as_tuple(keys_then_default_value...),
