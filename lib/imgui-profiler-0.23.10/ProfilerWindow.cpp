		...
		*/
		auto TrackHeader = [&](const char* pName, uint32 id)
		{
			...
			if (ImGui::ItemAdd(ImRect(trackTextCursor, trackTextCursor + ImVec2(caretSize, caretSize)), id))
			{
				...
				pDraw->AddText(ImGui::GetItemRectMin() + ImVec2(2, 2), ImColor(style.BGTextColor), isOpen ? "v" : ">");
...
void DrawProfilerHUD()
{
	...
	ImGui::SetNextItemWidth(150);
	ImGui::SameLine();
	ImGui::InputText("##Search", context.SearchString, std::size(context.SearchString));
	ImGui::SameLine();
	if (ImGui::Button("ClearFilter##clearfilter"))
		context.SearchString[0] = 0;
	ImGui::SameLine();
	if (ImGui::Button("StyleEdit##styleeditor"))
